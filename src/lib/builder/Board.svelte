<script>
  ...
  import { WORKSPACE_CONTEXT, BOARD_KEY, projectNodes, mergeBoardNodes, parseBoard, nodeWidth, nextNodePosition, newDraftNode } from './model.js';
  ...
  $effect(() => {
    ...
    const saved = nodes.filter((node) => node.type === 'project').map(({ id, position, data }) => ({ id, position, inputs: data.inputs }));
  ...
  function focusNode(node) {
    const width = nodeWidth(node, window.innerWidth < 700);
  ...
  function newProject(initial = null) {
    const node = newDraftNode(nextNodePosition(nodes), initial, Boolean(initial?.prompt) && shouldAutoStart);
    nodes = [...nodes, node];
    if (isMounted) focusNode(node);
  }
  function newSessionAt(position) {
    nodes = [...nodes, newDraftNode(position, null, false)];
    contextMenu.focusCanvas();
  }
  function openSession(session) {
    let node = nodes.find((node) => node.id === session.id);
    if (!node) { node = { id: session.id, type: 'project', dragHandle: '.node-handle', position: nextNodePosition(nodes), data: { session } }; nodes = [...nodes, node]; }
    showChats = false; focusNode(node);
  }
  function created(id, session, inputs) {
    ondraftconsumed();
    sessions = [session, ...sessions.filter((item) => item.id !== session.id)];
    nodes = nodes.map((node) => node.id === id ? { ...node, id: session.id, type: 'project', data: { session, inputs } } : node);
...
<div class="workspace-board" data-umami-mask>
  ...
  <div class="board-flow" bind:this={canvas}><SvelteFlowProvider><SvelteFlow tabindex={0} aria-label="Workspace canvas" aria-keyshortcuts="Shift+F10" onkeydown={(event) => contextMenu.keyboard(event)} onpanecontextmenu={(event) => contextMenu.open(event)} onmovestart={() => contextMenu.close()} bind:nodes bind:edges bind:viewport {nodeTypes} colorMode="dark" minZoom={0.25} maxZoom={1.5} panOnScroll zoomOnScroll={false} deleteKey={null} nodesConnectable={false} zoomOnDoubleClick={false}><CanvasMenu bind:this={contextMenu} {canvas} oncreate={newSessionAt} /><Background gap={24} size={1} color="#3b3b40" /><Controls orientation="horizontal" showLock={false} /></SvelteFlow></SvelteFlowProvider></div>
  {#if isLoading}<div class="board-status" role="status">Opening your cloud workspace…</div>{:else}<p class="board-hint">Right-click the canvas for a new session. Drag a project’s header to move it.</p>{/if}
</div>
